...
class TestResPartnerCurp(TransactionCase):
    ...
    def test_clearing_curp_removes_backing_record(self):
        partner = self.env['res.partner'].create(
            {
                'name': 'Cliente clear',
                'wgs_curp': 'MOPL900101MDFABC01',
            }
        )

        partner.write({'wgs_curp': False})

        self.assertFalse(partner.wgs_curp)
        self.assertFalse(
            self.env['wgs.partner.curp'].sudo().search([('partner_id', '=', partner.id)], limit=1)
        )
