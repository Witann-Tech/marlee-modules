...
class SaleOrder(models.Model):
    ...
    def wgs_audit_pos_access_consistency_for_pos(self, options=False):
        ...
        if Person is False:
            return {
                ...
                'site_ids': [],
                'site_names': [],
                'access_person_model_available': False,
                'checked_access_people': 0,
                'active_enabled_access_people': 0,
                'checked_partners': 0,
                'company_partner_people': 0,
                'out_of_company_people': 0,
                'access_state_counts': {},
                'active_access_state_counts': {},
                'active_managed': 0,
                'active_manual': 0,
                'active_multisite': 0,
                'stale_managed_access': 0,
                'configuration_warning': _('El modelo access_control.person no está disponible en este entorno.'),
                'rows': [],
            }
        ...
        if not pos_sites:
            summary = {
                ...
                'site_ids': [],
                'site_names': [],
                'access_person_model_available': True,
                'checked_access_people': 0,
                'checked_partners': 0,
                'active_enabled_access_people': 0,
                'company_partner_people': 0,
    ...
    def _build_pos_subscription_directory_status_item(self, today):
        ...
        elif close_deadline and today >= close_deadline:
            access_state = False
            native_state_key = 'closed'
            native_state_label = _('Cerrada')
            reason = _('La suscripción superó los días de gracia del plan recurrente y debe reinscribirse.')
        ...
        return {
            ...
            'auto_close_date': close_deadline.isoformat() if close_deadline else False,
    ...
    def _build_pos_subscription_status_item(self, today):
        ...
        should_force_auto_close = bool(close_deadline and today >= close_deadline)
